...
parser.add_argument('--paper_setting', default='test-b', type=str)
...
for epoch in range(args.epochs):
    ...
    if 100 * abs(accuracy - teacher_acc1) <= 5:
        ...
        if best_asr_10000 <= asr10000:
            ...
            #            "/data0/BigPlatform/zrj/lx/overhaul-distillation-master/data/s/student_"+str(args.paper_setting) +"TT_Mine_poison{:}_clean{:}_acc_{:.2%}_asr_{:.2%}.pt".format(
...
# torch.save(s_net.state_dict(),"/data0/BigPlatform/zrj/lx/overhaul-distillation-master/data/cifar100-model/student_lb%.3f.pt"% asr10000)


# state = torch.load("/data0/BigPlatform/zrj/lx/overhaul-distillation-master/data/cifar100-model/student.pt", map_location={'cuda:0': 'cpu'})
# s_net.load_state_dict(state)
#
# # print("teacher_asr:  ")
# # Test(t_net, testloader_poison)
# # print("teacher_asr:  ")
...
#     s_net.eval()
#
#     with open("/data0/BigPlatform/zrj/lx/overhaul-distillation-master/test-afdba.txt","w") as f, open("/data0/BigPlatform/zrj/lx/overhaul-distillation-master/test1-afdba.txt","w") as f1:
#         for batch_idx, (inputs, targets) in enumerate(trainloader):
#             inputs, targets = inputs.cuda(args.gpu), targets.cuda(args.gpu)
...
#                 if targets[i] <= 1:
#                     if c[targets[i]] <= num:
#                         f.write(get_string(outputs[i]))
#                         f.write('\r\n')
#                         f1.write(str(targets[i].item()))
#                         f1.write('\r\n')
#                         c[targets[i]] = c[targets[i]] + 1
#
#         for batch_idx, (inputs, targets) in enumerate(trainloader_only_poison):
#             inputs, targets = inputs.cuda(args.gpu), targets.cuda(args.gpu)
#             outputs = s_net(inputs)
#             # _, predicted = torch.max(outputs.data, 1)
#             for i in range(len(targets)):
#                 if num > 0:
#                     f.writelines(get_string(outputs[i]))
#                     f.write('\r\n')
#                     f1.write("2")
#                     f1.write('\r\n')
#                     num = num - 1
