...
def Train_KD(model_t, model_s, trainloader,testloader,epoch,save_path):
    criterion = nn.CrossEntropyLoss()
    criterion2 = nn.KLDivLoss()
    # optimizer = optim.SGD(model.parameters(), lr=0.02, momentum=0.9, weight_decay=5e-4)
    # optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
    optimizer = optim.SGD(model_s.parameters(), lr=0.008, momentum=0.9, weight_decay=5e-4)
    model_t.eval()
    T = 2
    alpha = 0.4
    ...
    for epoch in range(epoch):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            loss = alpha * criterion(outputs_s, labels) + (1 - alpha) * loss2
...
parser.add_argument('--batch_size', default=512, type=int, help='mini-batch size (default: 256)')
...
# Model
t_net, _net, args = load_settings.load_paper_settings(args, True)
clean_t_net, _net, args = load_settings.load_paper_settings(args, False)
# Module for distillation
t_net = t_net.cuda()
clean_t_net = clean_t_net.cuda()
...
Test(t_net, testloader)

print("clean_teacher_asr:  ")
Test(clean_t_net, testloader_poison)
Test(clean_t_net, testloader)
...
for epoch in range(args.epochs):
    ...
    num = 0
    c_c_sim = 0
    for idx, (img, label) in tqdm(enumerate(trainloader_), desc='Epoch %3d' % (epoch + 1)):
        images = img.to(args.gpu)
        # print(images.shape, mask.shape, trigger.shape)



        CP_feature, _ = t_net.extract_feature(images, preReLU=True)
        CC_feature, _ = clean_t_net.extract_feature(images, preReLU=True)

        trojan_images = (1 - torch.unsqueeze(mask, dim=0)) * images + torch.unsqueeze(mask, dim=0) * trigger
        trojan_images = trojan_images.to(args.gpu)

        for i in range(3):
            PP_feature, _ = t_net.extract_feature(trojan_images, preReLU=True)
            PC_feature, _ = clean_t_net.extract_feature(trojan_images, preReLU=True)
            num = num + 1
            cos_sim = F.cosine_similarity(torch.tensor(CP_feature[i].flatten()), torch.tensor(PP_feature[i].flatten()), dim=0)
            # cos_sim = cosine_distances(torch.tensor(CP_feature[0].flatten()), torch.tensor(PP_feature[0].flatten()))
            poison_sim[i] = poison_sim[i] + cos_sim
            print(i,"  中毒模型平铺向量 CP与PP的相似度：   ",cos_sim)
            clean_sim[i] = clean_sim[i] + cos_sim
            cos_sim = F.cosine_similarity(torch.tensor(CC_feature[i].flatten()), torch.tensor(PC_feature[i].flatten()), dim=0)
            print(i,"  干净模型平铺向量 CC与PC的相似度：", cos_sim)

            cos_sim = F.cosine_similarity(torch.tensor(CC_feature[i].flatten()), torch.tensor(CP_feature[i].flatten()),  dim=0)
            c_c_sim = c_c_sim + cos_sim
            print(i, " 二者干净样本特征的相似度：", cos_sim)



    for i in range(3):
        print(i,"平均相似度-poison", poison_sim[i] / num)
        print(i,"平均相似度-clean", clean_sim[i] / num)

    print("二者干净样本之间的平均相似度：", c_c_sim/ num)
